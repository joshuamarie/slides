...
viewof sigma_empirical = Inputs.range([0.5, 5], {
    label: "Standard Deviation (σ)",
    value: 1,
    step: 0.1
})
...
//| panel: fill
{
    ...
    const xMin = mu_empirical - 4 * sigma_empirical;
    const xMax = mu_empirical + 4 * sigma_empirical;
    
    // Generate normal distribution data
    const data = [];
    for (let x = xMin; x <= xMax; x += (xMax - xMin) / 1000) {
        const y = (1 / (sigma_empirical * Math.sqrt(2 * Math.PI))) * 
                  Math.exp(-0.5 * Math.pow((x - mu_empirical) / sigma_empirical, 2));
        data.push({x, y});
    }
    ...
    const yMax = (1 / (sigma_empirical * Math.sqrt(2 * Math.PI)));
    const yScale = d3.scaleLinear()
        .domain([0, yMax * 1.1])
        .range([height - margin.bottom, margin.top]);
    ...
    const createArea = (sigma_mult, color) => {
        const areaData = data.filter(d => 
            d.x >= mu_empirical - sigma_mult * sigma_empirical && 
            d.x <= mu_empirical + sigma_mult * sigma_empirical
        );
    ...
    if (ruleSelect === 3 || ruleSelect === 4) {
        createArea(3, "#C73E1D");
    }
    if (ruleSelect === 2 || ruleSelect === 4) {
        createArea(2, "#F18F01");
    }
    if (ruleSelect === 1 || ruleSelect === 4) {
        createArea(1, "#A23B72");
    }
    ...
    // Mean line
    drawVLine(mu_empirical, "red", "5,5");
    
    // Standard deviation lines
    if (ruleSelect === 1 || ruleSelect === 4) {
        drawVLine(mu_empirical - sigma_empirical, "#A23B72", "3,3");
        drawVLine(mu_empirical + sigma_empirical, "#A23B72", "3,3");
    }
    if (ruleSelect === 2 || ruleSelect === 4) {
        drawVLine(mu_empirical - 2 * sigma_empirical, "#F18F01", "3,3");
        drawVLine(mu_empirical + 2 * sigma_empirical, "#F18F01", "3,3");
    }
    if (ruleSelect === 3 || ruleSelect === 4) {
        drawVLine(mu_empirical - 3 * sigma_empirical, "#C73E1D", "3,3");
        drawVLine(mu_empirical + 3 * sigma_empirical, "#C73E1D", "3,3");
    }
